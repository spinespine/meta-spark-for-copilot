...
export class MetaRequestError extends Error {
	readonly kind: MetaRequestErrorKind;
...
export async function createHttpError(
	...
	return new MetaRequestError({
		message: `Meta API request failed with HTTP ${response.status}`,
		userSummary,
		kind: 'http',
		baseUrl,
...
export function normalizeRequestError(error: unknown, context: RequestErrorContext): Error {
	...
	if (!(error instanceof Error)) {
		...
		return new MetaRequestError({
			message: `Meta request failed with a non-Error value: ${value}`,
	...
	const enhanced = new MetaRequestError({
		message: code
			? `Meta request failed due to network error ${code}`
			: 'Meta request failed due to a network error',
		userSummary,
		kind: 'network',
		baseUrl: context.baseUrl,
		code,
...
function getHttpErrorMessage(status: number, serverCode: string | undefined, serverMessage: string | undefined, createApiKeyUrl?: string): string {
	...
	if (status === 400 && serverMessage) {
		return t('error.http.400.withMessage', status, serverMessage);
	}
	switch (status) {
		case 400:
...
function extractServerCode(responseText: string): string | undefined {
	try {
		...
		const error = getObjectProperty(parsed, 'error');
		return getStringProperty(error, 'code') ?? getStringProperty(parsed, 'code');
	} catch {
		return undefined;
	}
}
...
function identifyApiProvider(baseUrl: string): ApiProviderId | undefined {
	return isOfficialMetaBaseUrl(baseUrl) ? 'meta' : undefined;
}
