...
export function shouldForceThinkingNone(requestKind: RequestKind): boolean {
	return REQUEST_KINDS_WITH_FORCED_MINIMAL_THINKING.has(requestKind);
}

export function shouldForceMinimalThinking(requestKind: RequestKind): boolean {
	return REQUEST_KINDS_WITH_FORCED_MINIMAL_THINKING.has(requestKind);
}
...
export function classifyMetaRequest(input: {
	...
	return classifyRequest({
		firstText:
			getFirstMetaText(input.request.messages) ??
		...
		latestUserText:
			...
			getLatestMetaUserText(input.request.messages),
...
function getMetaToolName(tool: MetaTool): string {
