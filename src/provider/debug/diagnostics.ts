...
export interface CacheDiagnosticsRun {
	...
	onUsage(usage: MetaUsage, charsPerToken: number): void;
}
...
function getCacheHitRate(usage: MetaUsage): string {
	const cacheHit = usage.prompt_cache_hit_tokens ?? 0;
	const cacheMiss = usage.prompt_cache_miss_tokens ?? 0;
...
export function createCacheTraceSnapshot(
	request: MetaRequest,
	inputMessages: readonly vscode.LanguageModelChatRequestMessage[] = [],
	requestKind: RequestKind = classifyMetaRequest({ request, inputMessages }),
): CacheTraceSnapshot {
	...
	return {
		...
		requiresReasoningContent: (request as any).thinking?.type === 'enabled',
...
function createRedactedComparisonInput(
	...
	return stableStringify({
		...
		thinking: (request as any).thinking ?? null,
...
function summarizeMessages(messages: any[]): CacheTraceMessageSummary[] {
...
function summarizeTools(tools: any[]): CacheTraceToolSummary[] {
...
function summarizeStats(messages: any[], toolCount: number): CacheTraceStats {
	let userMessages = 0;
	...
	for (const message of messages) {
		...
		totalContentChars += getMetaContentLength(message);
		if (getMetaContentLength(message) > LARGE_MESSAGE_CHARS) {
			largeMessages += 1;
		}
		...
		if ((message.content as any).includes(IMAGE_DESCRIPTION_UNAVAILABLE)) {
