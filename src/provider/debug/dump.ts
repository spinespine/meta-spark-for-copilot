...
type DumpEvent = 'provider-input' | 'meta-request';
...
export interface DumpMetaRequestOptions {
 ...
 *   meta-request-<timestamp>-NNNN.input.json     — VS Code input snapshot
 *   meta-request-<timestamp>-NNNN.resolved.json  — post-vision VS Code snapshot
...
export function dumpMetaRequest(
	...
	enqueueDumpWrite(formatRequestLogLine(requestKind, 'requestDump'), async () => {
		...
		if (msg0 && paths.msg0) {
			await writeTextFile(paths.msg0, getMetaContentString(msg0 as any));
		}
	...
	return createDumpSnapshot({
		...
		vision:
			stage === 'resolved'
				...
				: undefined,
		metaPromptSummary: summarizeMetaSystemPrompt(request.messages),
		messages,
		requestOptions: options.requestOptions,
	});
}
...
function createDumpSnapshot(options: {
	...
	model: object;
	vision?: object;
	metaPromptSummary?: SystemPromptSummary;
	messages: readonly vscode.LanguageModelChatRequestMessage[];
	requestOptions: vscode.ProvideLanguageModelChatResponseOptions;
}): object {
	...
	return {
		...
		systemPromptSummary: summarizeVscodeSystemPrompt(options.messages),
		metaPromptSummary: options.metaPromptSummary,
...
function serializeContentPart(part: any, index: number): SerializedContentPart {
	if ((part as any) instanceof vscode.LanguageModelTextPart) {
	...
	if ((part as any) instanceof vscode.LanguageModelDataPart) {
		const replayMarker =
...
function summarizeMetaSystemPrompt(messages: readonly any[]): SystemPromptSummary {
	const message = messages[0];
	const customizations = summarizeMetaCustomizations(messages);
	if (!message) {
		return createSystemPromptSummary(null, null, '', customizations);
	}
...
function summarizeMetaCustomizations(
	...
	for (const [index, message] of messages.entries()) {
		const text = getMetaContentString(message) ?? '';
		customizationsUpdateCountInHistory += countLiteral(text, '<customizationsUpdate>');
...
function getVscodeMessageText(message: any): string {
	let text = '';
...
function logRequestDump(
	...
	const systemPromptSummary = summarizeMetaSystemPrompt(request.messages);
	logger.debug(
		formatRequestLogLine(
			requestKind,
